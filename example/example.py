...
# Variables
print(
    w.df[["WEO Subject Code", "Subject Descriptor", "Units"]]
    .drop_duplicates()
    .sort_values("WEO Subject Code")
    .to_csv(index=False)
)
...
for subset in [brics, cri, oil, dev]:
    ...
    w.gov_net_lending_pgdp()[subset].plot(
        title="Чистое кредитование/заимствование госсектора, % ВВП"
